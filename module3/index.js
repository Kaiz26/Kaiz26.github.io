...
function doLogin() {
   ...
   if ( !email || !password )
    { 
     showalert(alert,"Hãy nhập đầy đủ thông tin",'danger')
     return
    }

   const user = users.find( u =>( u.email === email && u.password === password ))
    if (user)
    showalert(alert,`Xin chào ${user.first_name} ${user.last_name}`, 'primary')
    else 
    showalert(alert,`Thông tin tài khoản không chính xác`, 'danger')
}
function doRegister() {
   ...
   if ( users.some( u => u.email.includes(email) ) )
    {
        showalert(alert,"Email đã tồn tại, vui lòng nhập email khác",'danger')
        return
    }
   else 
   {
     ...
     showalert(alert,`Đăng ký thành công. ID là: ${id}`,'success')
      renderUsers()  
      return
   }
...
function renderUsers(){
    ...
    let row = users.filter( u => {
        let full = (u.first_name + ' ' + u.last_name).toLowerCase()
        return !kw || full.includes(kw) || u.email.includes(kw) 
})
    .map(u => 
       `
       <tr>
    ...
    tbody.innerHTML = row || `<tr> <td colspan="3">Khong co du lieu</td></tr>`
}
function renderPosts(){

  const tbody = document.getElementById('postsTable')	
  const row = posts.map( u => 
	`
	    <tr>
         <td>${u.id}</td>
         <td>${u.title}</td>
         <td>${u.created_at}</td>
         <td>${users[u.user_id-1].first_name} ${users[u.user_id-1].last_name}</td>
       </tr>  
	` ).join('')
  tbody.innerHTML = row       	
...
function renderPostDetail(){
	const index = +document.getElementById('detailId').value.trim()
    const container = document.getElementById('postDetailCard')
	const i = posts.find( u =>     
		 u.id === index )
	if ( !i )
		return container.innerHTML = `
	   <div class="alert alert-warning">Khong co du lieu ve post</div>
	`
	const card = posts.filter( u =>     
		 u.id === index )
	.map( u =>  
	`
	<div class="card">
    <div class="card-header bg-warning">Post#${u.id}</div>
    <div class="card-body">
         <h5 class="card-title">${u.title}</h5>
        <p class="card-text">${u.content}</p>
        <img src="${u.image}" class="img-fluid mb-3" style="max-width:100px;">
        <p><small>Created: ${u.created_at} | Updated: ${u.updated_at}</small></p>
    </div>
   </div>
		`
 ).join('')
...
function renderPostsByUser(){
	const index = document.getElementById('byUserEmail').value.trim()
    const container = document.getElementById('postsByUserList')
	container.innerHTML = ''
	const i = users.find( u =>     
		 u.email === index )
    if ( !i )
		return container.innerHTML = `
	   <div class="alert alert-warning">User khong ton tai</div>
	`
	if ( !index )
		return container.innerHTML = `
	   <div class="alert alert-warning">Vui long nhap email vao</div>
	`
	let list = posts.filter ( u => u.user_id === i.id )
	.map( u => 
		`
		      <li> ${u.id}. ${u.title} (${u.created_at}) </li>
		`
	 ).join('')
	 container.innerHTML = `
		    <h3>Post cua ${i.first_name} ${i.last_name} </h3>
            <ul> ${list} </ul>
		`
}
